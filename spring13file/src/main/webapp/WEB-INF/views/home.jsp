...
<!-- 
	...
	GET방식은 용량제한 때문에 파일 저농이 불가능하며 이름만 전송
	POST방식은? 용량제한은 업지만 전송이 불가능하며 이름만 전송
	-> POST방식은 Encoding Type에 따라 두 가지로 나눠진다.
	 - application/x-www-form-urlencoded은 기본 방식이며 key=value 형태로 취급한다. 
	 - key와 value가 1:1 연결되므로 파일과 같이 정보가 많을 때는 전송이 어렵다.
	 - 파일을 전송하고 싶다면 반드시 enctype을 "multipart/form-data"로 변경해야 한다.(post방식)에서만 쓰인다. 
 -->

<!-- <form action="upload1"method="get" enctype="multipart/form-data"> -->
<!-- 	<input type="file"name="attact"> -->
<!-- 	<br><br> -->
<!-- 	<button>업로드</button> -->
<!-- </form> -->

<h2>POSTmultipart방식예제 </h2>
...
<!-- 
	...
	2. input 태그에 multiple 옵션을 추가한다 - 개수 무제한(제한하려면 프로그래밍)
 -->
